    ...
    public void log() {
        if (this.token != null && this.token.macroSub != null) {
            Logger.error(String.format("in file %s:%d:%d, symbol '%s' (expansion of macro %s at %s:%d:%d): %s",
                    this.token.location.filepath, this.token.location.lineNumber, this.token.location.colNumber, this.token.lexeme,
                    this.token.macroSub.lexeme, this.token.macroSub.location.filepath, this.token.macroSub.location.lineNumber, this.token.macroSub.location.colNumber, this.getMessage()));
        } else {
